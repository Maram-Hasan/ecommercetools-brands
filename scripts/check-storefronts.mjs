...
try {
  for (const [brand, storeKey] of [
    ...
    await page.screenshot({
      path: `test-results/live-${brand}-home.png`,
      fullPage: true,
    });
    if (brand === 'gh') {
      for (const width of [820, 768, 390, 320]) {
        ...
        assert(
          cta.y >= hero.y && cta.y + cta.height <= hero.y + hero.height,
          'Hero CTA must not be clipped',
        );
        await page.screenshot({
          path: `test-results/live-gh-home-${width}.png`,
          fullPage: true,
        });
      }
    ...
    const response = await page.request.get(
      `http://localhost:5173/api/products?store=${storeKey}`,
    );
    ...
    assert.equal(
      await page.locator('.product-info-panel h1').innerText(),
    ...
    await page.screenshot({
      path: `test-results/live-${brand}-pdp-desktop.png`,
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: `test-results/live-${brand}-pdp-mobile.png`,
      fullPage: true,
    });
    ...
    console.log(
      JSON.stringify({
        ...
        total: data.total,
        displayed,
        categories: data.products[0].categories?.length,
        staticImageRequests: demoRequests.length,
      }),
    );
    await page.close();
  }
